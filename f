...
local predictionFactor = 0.5 -- Higher value leads more, lower value is less aggressive
local predictionDistance = 20 -- Distance to predict ahead based on speed
local gravity = 196.2 -- Roblox gravity constant (for jumping)
...
-- Predict target position based on velocity and jumps
local function predictTargetPosition(targetPlayer)
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        if humanoid then
            ...
            local predictedPosition = humanoidRootPart.Position + velocity * predictionFactor * predictionDistance
            
            -- Adjust for gravity if the target is in the air (jumping or falling)
            if humanoidRootPart.Velocity.Y ~= 0 then
                local timeInAir = math.abs(humanoidRootPart.Velocity.Y) / gravity
                predictedPosition = predictedPosition + Vector3.new(0, -0.5 * gravity * timeInAir ^ 2, 0) -- Simple gravity prediction
            end
